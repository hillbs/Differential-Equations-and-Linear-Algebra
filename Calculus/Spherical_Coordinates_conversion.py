...
def spherical_to_cartesian(spherical_vect):
    ...
    """
    cart_vect = [0,0,0]
    # Trig alias. Creates the x-distance
    sin_phi = sin(spherical_vect[2])

    # The vector.
    cart_vect[0] = round((spherical_vect[0] * cos(spherical_vect[1]) * sin_phi), 12) # x value
    cart_vect[1] = round((spherical_vect[0] * sin(spherical_vect[1]) * sin_phi),12) # y value
    cart_vect[2] = round((spherical_vect[0] * cos(spherical_vect[2])), 12) #z value
    
    return cart_vect

k = spherical_to_cartesian([1,pi/4, pi/2])